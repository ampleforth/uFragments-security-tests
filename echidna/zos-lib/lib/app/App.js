...
class App {
  ...
  async setImplementation(contractClass, contractName) {
    ...
    const directory = this.currentDirectory();
    await (0, _Transactions.sendTransaction)(directory.setImplementation, [contractName, implementation.address], this.txParams);
  ...
  async unsetImplementation(contractName) {
    ...
    await (0, _Transactions.sendTransaction)(this.currentDirectory().unsetImplementation, [contractName], this.txParams);
    log.info(`Implementation unset`);
  }

  async setStdlib(stdlibAddress = 0x0) {
    log.info(`Setting stdlib ${stdlibAddress}...`);
    await (0, _Transactions.sendTransaction)(this.currentDirectory().setStdlib, [stdlibAddress], this.txParams);
  ...
  async newVersion(versionName, stdlibAddress = 0) {
    ...
    const directory = await (0, _Transactions.deploy)(AppDirectory, [stdlibAddress], this.txParams);
    log.info(` App directory: ${directory.address}`);
    await (0, _Transactions.sendTransaction)(this.package.addVersion, [versionName, directory.address], this.txParams);
    log.info(` Added version: ${versionName}`);
    await (0, _Transactions.sendTransaction)(this._app.setVersion, [versionName], this.txParams);
  ...
  async changeProxyAdmin(proxyAddress, newAdmin) {
    ...
    await (0, _Transactions.sendTransaction)(this._app.changeProxyAdmin, [proxyAddress, newAdmin], this.txParams);
  ...
  async _createProxy(contractName) {
    ...
    return (0, _Transactions.sendTransaction)(this._app.create, [contractName], this.txParams);
  }
  ...
  async _createProxyAndCall(contractClass, contractName, initMethodName, initArgs) {
    ...
    return (0, _Transactions.sendTransaction)(this._app.createAndCall, [contractName, callData], this.txParams);
  }

  async _upgradeProxy(proxyAddress, contractName) {
    log.info(`Upgrading ${contractName} proxy without running migrations...`);
    return (0, _Transactions.sendTransaction)(this._app.upgrade, [proxyAddress, contractName], this.txParams);
  }
  ...
  async _upgradeProxyAndCall(proxyAddress, contractClass, contractName, initMethodName, initArgs) {
    ...
    const callData = (0, _encodeCall2.default)(initMethodName, initArgTypes, initArgs);
    return (0, _Transactions.sendTransaction)(this._app.upgradeAndCall, [proxyAddress, contractName, callData], this.txParams);
  }

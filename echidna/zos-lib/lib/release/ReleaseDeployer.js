...
class ReleaseDeployer {
  ...
  async deployRelease() {
    ...
    this.release = await (0, _Transactions.deploy)(Release, [], this.txParams);
  ...
  async deployAndRegisterContracts(contracts, deployMethod) {
    await Promise.all(contracts.map(async contract => {
      ...
      await (0, _Transactions.sendTransaction)(this.release.setImplementation, [contractAlias, implementation.address], this.txParams);
    }));
  }
